...
class MainWindow(QMainWindow, windowUI):
    ...
    def find_corners(self):
        ...
        self.images_Q1 = utils.readImages("Q1_Image")
    ...
    def word_on_board(self):
        if len(self.images_Q2) == 0:
            self.images_Q2 = utils.readImages("Q2_Image")
        fs = cv.FileStorage("Q2_Image/Q2_Lib/alphabet_lib_onboard.txt", cv.FILE_STORAGE_READ)
    ...
    def word_Vertical(self):
        if len(self.images_Q2) == 0:
            self.images_Q2 = utils.readImages("Q2_Image")
        fs = cv.FileStorage("Q2_Image/Q2_Lib/alphabet_lib_vertical.txt", cv.FILE_STORAGE_READ)
        string = self.edit_2.text()[:6]
        if not string.isupper():
            string = string.upper()
        if not self.q2_calib:
    ...
    def keypoints_Q4(self):
        if len(self.images_Q4) == 0:
            self.images_Q4 = utils.readImages("Q4_Image", "*.jpg")
